...
console.log("🐕 Starting Barkend - Your Ultimate Dog Paradise!\n");

// Check if package.json exists
if (!fs.existsSync("package.json")) {
  console.log("❌ Error: package.json not found");
  console.log("Please run this script from the Barkend project directory\n");
  process.exit(1);
}
...
function startServers() {
  console.log("🚀 Starting Barkend servers...\n");

  // Start backend server
  console.log("🔧 Starting backend server on port 3000...");
  const backend = spawn("node", ["barkend-server.js"], {
    ...
    function tryNextCommand(index = 0) {
      if (index >= frontendCommands.length || frontendStarted) {
        if (!frontendStarted) {
          ...
          console.log(
            "Please manually serve the frontend directory on port 8080",
          );
  ...
  setTimeout(() => {
    ...
    console.log(
      "   • Frontend not loading? Try http://localhost:8080 manually",
    );
