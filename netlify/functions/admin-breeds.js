...
const ADMIN_KEY = 'barkend-admin-2024';
...
// GET - Retrieve all breeds with admin details
...
async function handlePost(event) {
  ...
  const existingBreed = await BreedCache.findOne({ breed: breedData.breed.toLowerCase() });
  if (existingBreed) {
    return {
      statusCode: 409,
      headers: {
...
async function handlePut(event) {
  ...
  const breed = await BreedCache.findOne({ breed: updateData.breed.toLowerCase() });
  if (!breed) {
    return {
      statusCode: 404,
      headers: {
  ...
  if (updateData.isActive !== undefined) breed.metadata.isActive = updateData.isActive;
...
async function handleDelete(event) {
  ...
  // Find breed
  const breed = await BreedCache.findOne({ breed: breedToDelete.toLowerCase() });
  if (!breed) {
    return {
      statusCode: 404,
      headers: {
