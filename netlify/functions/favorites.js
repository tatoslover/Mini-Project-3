...
const { FavoriteImage, ServerStats, ApiUsage, User } = require('../../models/models');
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    switch (event.httpMethod) {
      case 'GET':
        return await handleGetFavorites(userId, sessionId, headers, responseTime, event);

      case 'POST':
        return await handleAddFavorite(userId, sessionId, headers, responseTime, event);

      case 'DELETE':
        return await handleDeleteFavorite(userId, sessionId, headers, responseTime, event);
...
async function handleGetFavorites(userId, sessionId, headers, responseTime, event) {
  try {
...
// Add a new favorite
async function handleAddFavorite(userId, sessionId, headers, responseTime, event) {
  try {
    ...
    const existingFavorite = await FavoriteImage.findOne({ userId, imageUrl: url });
    if (existingFavorite) {
      return {
        statusCode: 409,
        headers,
...
// Delete a favorite
async function handleDeleteFavorite(userId, sessionId, headers, responseTime, event) {
  try {
