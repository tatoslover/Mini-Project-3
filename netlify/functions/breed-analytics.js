...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const popularityRank =
      allBreedViews.findIndex((b) => b._id === breed.toLowerCase()) + 1;
    ...
    // Find peak hour
    const peakHour =
      hourlyDistribution.reduce((peak, current) => {
    ...
    const analytics = {
      ...
      data: {
        ...
        trends: {
          ...
          peakHour,
          avgResponseTime:
            Math.round((usageStats[0]?.avgResponseTime || 0) * 10) / 10,
        },
        metadata: {
    ...
    const errorResponse = {
      ...
      breed:
        event.path.split("/")[event.path.split("/").length - 2] || "unknown",
      timestamp: new Date().toISOString(),
    };
