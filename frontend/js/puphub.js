class PupHubApp {
    constructor() {
        this.breeds = {};
        this.favorites = JSON.parse(localStorage.getItem('pupHubFavorites')) || [];
        this.stats = JSON.parse(localStorage.getItem('pupHubStats')) || {
            imagesViewed: 0,
    ...
    async loadBreeds() {
        try {
            ...
            if (data.status === 'success') {
                ...
                document.getElementById('totalBreedsCount').textContent = Object.keys(this.breeds).length;
            }
        } catch (error) {
    ...
    async displayBreeds() {
        ...
        if (breedsToShow.length === 0) {
            container.innerHTML = '<div class="empty-state"><i class="fas fa-search"></i><h3>No breeds found</h3><p>Try searching for something else.</p></div>';
            return;
        }
    ...
    async createBreedCard(breed) {
        try {
            ...
            if (data.status === 'success') {
                const breedName = breed.replace('/', ' ').split(' ').map(word =>
                ...
                return `
                    <div class="dog-card" onclick="pupHub.showBreedDetails('${breed}')">
                        <img src="${data.message}" alt="${breedName}" class="dog-image"
                             onerror="this.src='https://via.placeholder.com/300x200?text=Image+Not+Found'"
                             onload="pupHub.incrementImageViews()">
                        <div class="dog-name">${breedName}</div>
                        <div class="dog-info">Click to see more images</div>
                        <div class="dog-actions">
                            <button class="btn btn-primary" onclick="event.stopPropagation(); pupHub.showBreedDetails('${breed}')">
                                <i class="fas fa-images"></i> Gallery
                            </button>
                            <button class="btn btn-secondary" onclick="event.stopPropagation(); pupHub.toggleFavorite('${breed}')"
                                    style="${heartColor}">
    ...
    async searchBreeds() {
        const query = document.getElementById('breedSearch').value.toLowerCase().trim();
    ...
    async loadRandomDogs() {
        ...
        try {
            ...
            container.innerHTML = randomDogs.map((dog, index) => `
                <div class="dog-card">
                    ...
                    <div class="dog-actions">
                        <button class="btn btn-accent" onclick="pupHub.openImageModal('${dog.message}', 'Random Pup #${index + 1}')">
                ...
                const modalContent = `
                    <div style="margin-bottom: 20px;">
                        ...
                        <button class="btn ${isFavorite ? 'btn-secondary' : 'btn-primary'}"
                                onclick="pupHub.toggleFavorite('${breed}'); pupHub.showBreedDetails('${breed}')">
                    ...
                    <div class="breed-images">
                        ${images.map(img => `
                            <img src="${img}" alt="${breedName}" class="breed-image"
                                 onclick="pupHub.openImageModal('${img}', '${breedName}')"
    ...
    toggleFavorite(breed) {
        ...
        localStorage.setItem('pupHubFavorites', JSON.stringify(this.favorites));
        this.updateStats();
        this.loadFavorites();
    }
    ...
    setupPagination() {
        ...
        if (this.currentPage > 1) {
            paginationHTML += `
                <button class="page-btn" onclick="pupHub.changePage(${this.currentPage - 1})">
    ...
    showError(message) {
        ...
        if (activeTab) {
            const container = activeTab.querySelector('.dog-grid, #randomDogs, #favoritesList');
            if (container) {
    ...
    updateStats() {
        document.getElementById('imagesViewedCount').textContent = this.stats.imagesViewed;
        document.getElementById('favoritesCount').textContent = this.favorites.length;
        localStorage.setItem('pupHubStats', JSON.stringify(this.stats));
    }
    ...
    async loadPopularBreeds() {
        ...
        const popularBreeds = ['labrador', 'golden/retriever', 'bulldog/english', 'beagle', 'poodle/standard'];
    ...
    switch(tabName) {
        case 'random':
            if (document.getElementById('randomDogs').innerHTML.trim() === '') {
                pupHub.loadRandomDogs();
            }
            break;
        case 'favorites':
            pupHub.loadFavorites();
            break;
        case 'stats':
            pupHub.loadPopularBreeds();
            break;
    }
}
